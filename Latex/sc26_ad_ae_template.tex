...
The paper's runtime figures (Figures~4, 8--11) have two dedicated sibling drivers: \texttt{bash Figures/plot\_paper\_snapshot.sh} regenerates them from the frozen paper-canonical CSVs committed under \texttt{PaperSnapshot/<exp>/results/} and writes outputs into \texttt{Figures/GeneratedFigures/Runtime/}; \texttt{bash Figures/plot\_results.sh} regenerates them from each reviewer's locally-produced CSVs in \texttt{Experiments/<exp>/results/} (after \texttt{sbatch run\_\{daint,beverin\}.sh}) and writes the PDF/PNG figures \emph{next to the CSVs that drove them}, inside the same \texttt{Experiments/<exp>/results/} folder.
Because the two destinations are distinct, reviewers running both scripts can compare their figures against the paper side-by-side without either overwriting the other.
