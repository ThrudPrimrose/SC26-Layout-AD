...
The same driver \texttt{bash Figures/plot\_all.sh} is also the single entry point for the paper's runtime figures (Figures~4, 8--11): it invokes each \texttt{Experiments/<exp>/plot\_paper.py} twice -- once from \texttt{PaperSnapshot/<exp>/} (the frozen, paper-canonical CSVs committed alongside the artifact), gathered into \texttt{Figures/GeneratedFigures/Runtime/}, and once from \texttt{Experiments/<exp>/} (the fresh local reproduction), gathered into \texttt{Figures/GeneratedFigures/Runtime/new/}.
Reviewers comparing these two flat folders side-by-side can see at a glance whether their reproduction matches the submitted figures.
A reviewer who only wants the runtime figures can pass the subset name:
\texttt{bash Figures/plot\_all.sh Runtime}; \texttt{bash Figures/plot\_all.sh Peaks} refreshes the canonical \texttt{Experiments/common/stream\_peak.json} (STREAM peak values consumed by every per-experiment plot for bandwidth normalization).
