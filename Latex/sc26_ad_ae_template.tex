...
Contribution~$C_1$ is a pen-and-paper proof with no computational artifact; the illustrative quotient-graph figures (Figures~2--3) are produced by pure-matplotlib helper scripts in the \texttt{Figures/} folder of~R1.
The paper's runtime figures (Figures~4, 8--13) have two dedicated sibling drivers that iterate over E1--E5 and every \texttt{E6\_VelocityTendencies/loopnest\_\{1..6\}}: \texttt{bash Figures/plot\_paper\_snapshot.sh} regenerates them from the frozen paper-canonical CSVs committed under \texttt{PaperSnapshot/<exp>/results/} and writes outputs into \texttt{Figures/GeneratedFigures/Runtime/}; \texttt{bash Figures/plot\_results.sh} regenerates them from each reviewer's locally-produced CSVs in \texttt{Experiments/<exp>/results/} (after \texttt{sbatch run\_\{daint,beverin\}.sh}) and writes the PDF/PNG figures \emph{next to the CSVs that drove them}, inside the same \texttt{Experiments/<exp>/results/} folder.
Because the two destinations are distinct, reviewers running both scripts can compare their figures against the paper side-by-side without either overwriting the other; any experiment whose CSVs are missing in the relevant root is skipped with a one-line notice so the sweep never aborts on a missing dataset.
All three drivers export \texttt{MATPLOTLIBRC=Figures/matplotlibrc}, pinning every rendered figure to DejaVu~Sans to avoid silent STIX / Computer~Modern fallbacks on stock environments.
The umbrella driver \texttt{bash Figures/plot\_all.sh} invokes both of the above plus the illustrative groups; it also accepts subset names (\texttt{PaperSnapshot}, \texttt{Results}, \texttt{Runtime}, \texttt{Peaks}, or an illustrative group) as arguments.
\texttt{Peaks} refreshes the canonical \texttt{Experiments/common/stream\_peak.json} (STREAM peak values consumed by every per-experiment plot for bandwidth normalization).
...
After canonicalization these collapse into 6~pattern classes, and the downstream layout search operates on a 5-group storage taxonomy (\texttt{cv}, \texttt{ch}, \texttt{f}, \texttt{s}, \texttt{n}) that matches the reference DaCe sweep in \texttt{permute\_stage8.py}.
...
\item \emph{Loopnests~2--6:} Representative nests from the remaining five pattern classes (direct stencil / partial vertical, direct stencil / complete vertical, indirect stencil + CFL clip / partial vertical, horizontal-only boundary, and vertical-only level reduction) are each implemented as a full C++/OpenMP + CUDA/HIP benchmark suite (single-source via \texttt{common/gpu\_compat.cuh}) with an analytic cost-metric sweep.
Each suite sweeps V1--V7 dimension-order variants and, where the access pattern makes it meaningful, blocked factors $B \in \{8,16,32,64,128\}$ and 2D tiling over $(T_X, T_Y)$; GPU variants add a thread-block-size sweep.
...
The evaluation is driven by \texttt{build\_layout\_crossproduct.py} (which projects each micro-bench V-winner onto its per-axis storage component --- IC for (h,v,b)~arrays, IN for 2-column connectivity --- and votes per group on axis values rather than opaque V-ids) followed by \texttt{run\_stage\{4,8\}\_permutations.py}, which runs each configuration in \texttt{layout\_crossproduct.csv} per device and per loop-nest lowering stage.
...
\item[$T_{6.5}$] \textbf{Blocked-vs-unblocked verdict:} \texttt{analyze\_blocked\_vs\_unblocked.py} compares the best blocked layout's median runtime against the best unblocked layout's median, per loopnest $\times$ target.
A loopnest \emph{keeps} blocked in its candidate pool iff the max speedup across the selected backends meets \texttt{--threshold} (default $1.25\times$); otherwise blocked is dropped.
Writes \texttt{blocked\_verdict.json}.

\item[$T_{6.6}$] \textbf{Layout cross-product:} \texttt{build\_layout\_crossproduct.py} ranks V-ids per loopnest by the geomean of normalized medians across every measured $\{$platform, backend$\}$ target, then \emph{projects} each V-id onto its storage-axis components (e.g.\ V6 $\to$ IC:v\_first, IN:AoS) and votes for axis values on the axis each group consumes.
Each group's candidate set is the rank-weighted winner on its axis (rank-1 union, padded to \texttt{--top-k}); unanimous groups collapse to a single candidate and groups with no touching loopnest (e.g.\ \texttt{n}) fall back to both axis values.
Writes \texttt{full\_velocity\_tendencies/layout\_crossproduct.\{json,csv\}} --- the input list for the full-module sweep, with one row per $(\mathrm{cv},\mathrm{ch},\mathrm{f},\mathrm{s},\mathrm{n})$ axis-value tuple.

\item[$T_{6.7}$] \textbf{Full-module sweep:} \texttt{full\_velocity\_tendencies/} applies each configuration from \texttt{layout\_crossproduct.csv} to the velocity-tendencies DaCe SDFG, drives the stage-by-stage lowering pipeline (stage1~$\to$~stage9), and benchmarks the terminal stage (stage4 on CPU, stage8 on GPU).
...
\item[$T_{6.8}$] \textbf{Analysis:} \texttt{plot\_paper.py} (top-level) and the per-folder plotting scripts under \texttt{full\_velocity\_tendencies/scripts/plotting/} produce Figures~12--13 and Table~IV from the combined results of $T_{6.4}$ and $T_{6.7}$.
\end{description}

\noindent Dependencies: $T_{6.1} \to T_{6.2} \to T_{6.3} \to T_{6.4} \to T_{6.5} \to T_{6.6} \to T_{6.7} \to T_{6.8}$.
...
\begin{itemize}
\item \texttt{access\_analysis/}: implements $T_{6.2}$ and $T_{6.3}$.
Parses the preprocessed velocity-tendencies source (\texttt{velocity\_advection\_preprocessed.f90}) with LOKI, canonicalizes the 25~loop nests into 6~pattern classes (\texttt{select\_loopnests.py} picks one representative per class), and derives the 5-group storage taxonomy (cv / ch / f / s / n with per-group axis tag) via \texttt{generate\_canonical\_groups.py}.
Outputs: \texttt{layout\_candidates.json}, \texttt{chosen\_loopnests.md}, \texttt{canonical\_array\_groups.json}.

\item \texttt{loopnest\_\{1..6\}/}: one subfolder per pattern class, implementing $T_{6.4}$.
All six are complete C++/OpenMP + CUDA/HIP benchmark suites (single-source via \texttt{common/gpu\_compat.cuh}) with matching analytic cost metrics.
...
\item \texttt{analyze\_blocked\_vs\_unblocked.py} + \texttt{build\_layout\_crossproduct.py} (E6 root): implement $T_{6.5}$ and $T_{6.6}$.
\texttt{analyze\_blocked\_vs\_unblocked.py} emits the blocked-vs-unblocked verdict (JSON); \texttt{build\_layout\_crossproduct.py} consumes it together with the per-loopnest CSVs and the canonical taxonomy to produce \texttt{full\_velocity\_tendencies/layout\_crossproduct.\{json,csv\}} --- the axis-projected cross-product that drives the full-module sweep.

\item \texttt{full\_velocity\_tendencies/}: implements $T_{6.7}$.
