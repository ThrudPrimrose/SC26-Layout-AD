...
int main(int argc, char **argv) {
    const char *csv_path = (argc > 1) ? argv[1] : "results_gpu_oop.csv";
...

    {
        ...
        GPU_CHECK(cudaMemcpy(di, h, bytes, cudaMemcpyHostToDevice));
        free(h);
    }
    ...
    GPU_CHECK(cudaFree(di)); GPU_CHECK(cudaFree(dout));
    fclose(csv);
