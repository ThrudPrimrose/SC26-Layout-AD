...
|---|---|---|
| DaCe | `f2dace/staging` | Required for E8's codegen pipeline. The run script exports `DACE_BRANCH=f2dace/staging` before sourcing `activate.sh`, which then ensures the checkout is on that branch. E1..E6 don't import dace and don't trigger any branch switch. E7 (WIP) pins `yakup/dev`; do not run E7 and E8 concurrently against the same DaCe clone. |
