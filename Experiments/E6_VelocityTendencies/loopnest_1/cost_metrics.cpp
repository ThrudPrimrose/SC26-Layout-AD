...
/* Tiled storage layout (TX × TY). X-tile decomposition matches IC_b when
 * TY == nl, so NUMA / block-address logic reuses the blocked (B=TX) path. */
inline int IC_t(int je, int jk, int TX, int TY, int nl) {
  int n_y_tiles = (nl + TY - 1) / TY;
  int x_tile = je / TX;
  int y_tile = jk / TY;
  int x_in  = je - x_tile * TX;
...
/* Tiled refs: same index-array layout as blocked (X-only), 2D tile for
 * data arrays via IC_t. */
inline void refs_t(int TX, int TY, int je, int jk, int N, int nl,
                   const int *ci, const int *vi, Ref r[NR]) {
  int c = IC_t(je, jk, TX, TY, nl);
  int c0 = ci[IN_b(je, 0, TX)], c1 = ci[IN_b(je, 1, TX)],
      v0 = vi[IN_b(je, 0, TX)], v1 = vi[IN_b(je, 1, TX)];
  r[0]  = {A_OUT,       blk(c, A_OUT)};
  r[1]  = {A_VN_IE,     blk(c, A_VN_IE)};
  r[2]  = {A_INV_DUAL,  blk(je, A_INV_DUAL)};
  r[3]  = {A_W,         blk(IC_t(c0, jk, TX, TY, nl), A_W)};
  r[4]  = {A_W,         blk(IC_t(c1, jk, TX, TY, nl), A_W)};
  r[5]  = {A_Z_VT_IE,   blk(c, A_Z_VT_IE)};
  r[6]  = {A_INV_PRIMAL,blk(je, A_INV_PRIMAL)};
  r[7]  = {A_TANGENT,   blk(je, A_TANGENT)};
  r[8]  = {A_Z_W_V,     blk(IC_t(v0, jk, TX, TY, nl), A_Z_W_V)};
  r[9]  = {A_Z_W_V,     blk(IC_t(v1, jk, TX, TY, nl), A_Z_W_V)};
  r[10] = {A_CELL_IDX,  blk(IN_b(je, 0, TX), A_CELL_IDX)};
  r[11] = {A_CELL_IDX,  blk(IN_b(je, 1, TX), A_CELL_IDX)};
  r[12] = {A_VERT_IDX,  blk(IN_b(je, 0, TX), A_VERT_IDX)};
  r[13] = {A_VERT_IDX,  blk(IN_b(je, 1, TX), A_VERT_IDX)};
}
...
static inline void step(int W, int N, int nl, LoopOrder lo,
                        ...
                        int TX = 0, int TY = 0) {
  /* Tiled path reuses blocked NUMA/weight logic with B_eff = TX. */
  int B_eff = (TY > 0) ? TX : B;
  curr.clear();
  for (int w = 0; w < W; w++) {
    ...
    Ref r[NR];
    if (TY > 0)     refs_t(TX, TY, je, jk, N, nl, ci, vi, r);
    else if (V > 0) refs_v(V, je, jk, N, nl, ci, vi, r);
...
static Metrics compute_slice_tiled(int TX, int TY, int W, int N, int nl,
  ...
  for (int64_t xb = lo_xb; xb < hi_xb; xb++)
    for (int yb = 0; yb < n_y_tiles; yb++)
      for (int y_in = 0; y_in < TY; y_in++) {
        int jk = yb * TY + y_in;
        if (jk >= nl) continue;
        for (int j = 0; j + W <= TX; j += W) {
          int je = (int)xb * TX + j;
          step(W, N, nl, KLON_FIRST, je, jk,
               ci, vi, prev, curr, acc, 0, 0, TX, TY);
        }
      }
  if (!acc.T) return {};
  double d = (double)acc.T;
  ...
  double delta_numa_val = acc.sdnuma / d;
  double delta_max_val  = acc.sdmax  / d;
  return { mu_val, delta_val, delta_numa_val, delta_max_val,
           mu_val * delta_val, mu_val * delta_numa_val, acc.T };
}
...
static Metrics compute_metrics_tiled(int TX, int TY, int W, int N, int nl,
  ...
  for (int d = 0; d < P_NUMA; d++) {
    ...
    if (l >= h) continue;
    NU_HOME = d;
    auto m = compute_slice_tiled(TX, TY, W, N, nl, ci, vi, l, h);
    add(m);
  }
  if (!nd) return {};
  double dn = (double)nd;
...
struct Res {
  ...
  int V, B, bb, w;
  int TY;               /* storage tile-Y; 0 for non-tiled rows (nl match) */
  double l1r;
  Metrics m;
};
...
int main(int argc, char **argv) {
  const char *csv_path_arg = (argc > 1) ? argv[1] : nullptr;
  ...
  bool hex = icon_load_patch(patch_path.c_str(), N, ied);

  int ndists = hex ? 5 : 4;
  Schedule scs[] = {SCHED_OMP_FOR, SCHED_OMP_COLLAPSE2};
  std::vector<Res> res;

  std::string csv_default = "metrics" + std::to_string(nl) + ".csv";
  const char *csv_path = csv_path_arg ? csv_path_arg : csv_default.c_str();
  FILE *csv = fopen(csv_path, "w");
  fprintf(csv,
    "nlev,variant,blocking,tile_y,cell_dist,schedule,target,block_bytes,vector_width,"
      ...
      for (int si = 0; si < 2; si++)
        for (int di = 0; di < ndists; di++)
          for (int ti = 0; ti < NT; ti++) {
            ...
            loc.push_back({tgts[ti].c, sched_name[si], dname[di],
                           0, B, tgts[ti].bb, Wv, 0, ws, m});
          }
    #pragma omp critical
    res.insert(res.end(), loc.begin(), loc.end());
  }

  /* ── Tiled (TX × TY) storage layout ── */
  static const int TX_SZ[] = {8, 16, 32, 64};
  static const int TY_SZ[] = {8, 16, 32, 64};
  constexpr int NTX = sizeof(TX_SZ)/sizeof(TX_SZ[0]);
  constexpr int NTY = sizeof(TY_SZ)/sizeof(TY_SZ[0]);
  #pragma omp parallel
  {
    std::vector<Res> loc;
    #pragma omp for schedule(static) collapse(4)
    for (int txi = 0; txi < NTX; txi++)
      for (int tyi = 0; tyi < NTY; tyi++)
        for (int di = 0; di < ndists; di++)
          for (int ti = 0; ti < NT; ti++) {
            ...
            std::vector<int> ci(cN * 2), vi(cN * 2);
            if (di < 4) {
              gen_ci_b(ci.data(), cN, TX, (CellDist)di, rng);
              gen_vi_b(vi.data(), cN, TX, rng);
            } else {
              for (int je = 0; je < cN; je++) {
                ci[IN_b(je, 0, TX)] = ied.cell_idx[je * 2];
                ci[IN_b(je, 1, TX)] = ied.cell_idx[je * 2 + 1];
                vi[IN_b(je, 0, TX)] = ied.vert_idx[je * 2];
                vi[IN_b(je, 1, TX)] = ied.vert_idx[je * 2 + 1];
              }
            }
            BLOCK_BYTES_G = tgts[ti].bb;
            Metrics m = compute_metrics_tiled(TX, TY, Wv, cN, nl,
                                              ci.data(), vi.data());
            /* Working-set heuristic: still driven by X-tile footprint. */
            double ws = ((double)TX * (8 * 8 + 4 * 4)) / (double)L1_BYTES;
            loc.push_back({tgts[ti].c, "tiled_omp_for", dname[di],
                           0, TX, tgts[ti].bb, Wv, TY, ws, m});
          }
    #pragma omp critical
    res.insert(res.end(), loc.begin(), loc.end());
  }
  ...
  for (auto &r : res) {
    ...
    fprintf(csv,
      ...
      "%.4f,%.4f,%.4f,%d,%ld\n",
      nl, r.V, r.B, r.TY, r.dist, r.sch, r.tgt, r.bb, r.w,
  ...
  printf("\n  N=%d nl=%d beta=%.4f alpha=%.4f gamma=%.3f P=%d L1=%d\n\n",
         N, nl, BETA, ALPHA, GAMMA, P_NUMA, L1_BYTES);
  printf("  %-4s %-8s %-11s %-13s %-11s %8s %8s %8s %8s %8s %8s %8s\n",
         "V", "B/tile", "Target", "Sched", "Dist",
  ...
  for (auto &r : res) {
    ...
    if (r.TY > 0)     snprintf(bl, 12, "%dx%d", r.B, r.TY);
    else if (r.B > 0) snprintf(bl, 12, "%d", r.B);
    else              snprintf(bl, 12, "-");

    printf("  %-4s %-8s %-11s %-13s %-11s %s %s %s %s %s %s %s\n",
