#!/usr/bin/env bash
#SBATCH --job-name=E6L1_zvgradw_beverin
#SBATCH --nodes=1
...
#SBATCH --exclusive
#SBATCH --chdir=.
#SBATCH --output=results/beverin/E6L1_zvgradw_beverin_%j.out
#SBATCH --error=results/beverin/E6L1_zvgradw_beverin_%j.err
#
# E6 / loopnest_1 -- z_v_grad_w indirect-stencil sweep on Beverin.
...
echo "[E6L1 beverin] host=$(hostname) threads=$OMP_NUM_THREADS data=$ICON_DATA_PATH"
...
${GPU_CXX} ${GPU_CXXFLAGS} -fgpu-rdc  -o bench_gpu_oldstyle -x hip bench_gpu_oldstyle.cu    ${GPU_LDFLAGS}

# --- T3 run (binaries write CSV at the path passed as argv[1]) -----------
./bench_cpu_a        results/beverin/z_v_grad_w_cpu.csv
./bench_gpu_a        results/beverin/z_v_grad_w_gpu.csv
./bench_gpu_oldstyle results/beverin/z_v_grad_w_gpu_old.csv
...
echo "[E6L1 beverin] done. CSVs under results/beverin/"
