...
struct GpuFlush {
    ...
    void flush(){init(); dim3 bl(16,16),gr((FLUSH_N+15)/16,(FLUSH_N+15)/16);
        ...
        /* Deterministic sink: stride-primed fixed index prevents compiler
         * from eliding the flush without introducing nondeterminism. */
    ...
    double* icon_inv_dual, double* icon_inv_primal, double* icon_tangent)
{
    ...
    CUDA_CHECK(cudaMalloc(&d_vn,sz_e*8)); CUDA_CHECK(cudaMalloc(&d_w,sz_c*8));
    CUDA_CHECK(cudaMalloc(&d_vt,sz_e*8)); CUDA_CHECK(cudaMalloc(&d_zw,sz_v*8));
    CUDA_CHECK(cudaMalloc(&d_out,sz_e*8));
    CUDA_CHECK(cudaMalloc(&d_id,N_e*8)); CUDA_CHECK(cudaMalloc(&d_ip,N_e*8));
