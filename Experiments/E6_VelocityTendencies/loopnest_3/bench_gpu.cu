...
int main(int argc, char *argv[]) {
  ...
  CUDA_CHECK(cudaMalloc(&d_w,   sz*8)); CUDA_CHECK(cudaMalloc(&d_vie, sz*8));
  CUDA_CHECK(cudaMalloc(&d_vti, sz*8));
  CUDA_CHECK(cudaMalloc(&d_gh, NLEVS[0]*8)); CUDA_CHECK(cudaMalloc(&d_iv, NLEVS[0]*8));
