#!/usr/bin/env bash
#SBATCH --job-name=E6L3_zvgradwfull_daint
#SBATCH --nodes=1
...
#SBATCH --exclusive
#SBATCH --output=results/daint/E6L3_zvgradwfull_daint_%j.out
#SBATCH --error=results/daint/E6L3_zvgradwfull_daint_%j.err
#
# E6 / loopnest_3 -- z_v_grad_w direct stencil (full vertical) sweep on Daint.Alps.
...
echo "[E6L3 daint] host=$(hostname) threads=$OMP_NUM_THREADS data=$ICON_DATA_PATH"
...
./bench_cpu_a results/daint/z_v_grad_w_full_cpu.csv
./bench_gpu_a results/daint/z_v_grad_w_full_gpu.csv
...
echo "[E6L3 daint] done. CSVs under results/daint/"
