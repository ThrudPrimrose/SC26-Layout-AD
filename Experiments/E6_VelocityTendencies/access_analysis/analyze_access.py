...
-------------------
  ...
  behavior:   compute, accumulate (true reduction: LHS missing a loop dim), reduction (MAX/MIN)
"""
...
class LoopNestInfo:
    ...
    @property
    def _collapsed_su_no_block(self):
        """Collapsed S/U with block dimension stripped."""
        parts = [p.strip() for p in self.collapsed_su.split("  ") if p.strip()]
        filtered = [p for p in parts if not p.startswith("b:")]
        return "  ".join(filtered) if filtered else "-"

    @property
    def _shape_no_block(self):
        """Loop shape with block dimension stripped."""
        parts = [r for r, v in zip(self.loop_shape.split("."), self.vars) if _role(v) != "b"]
        return ".".join(parts) if parts else "h"

    @property
    def _ranges_no_block(self):
        """Ranges with block dimension stripped."""
        return tuple(r for r, v in zip(self.ranges, self.vars) if _role(v) != "b")

    @property
    def pattern_key(self):
        """Pattern key with block dimension stripped."""
        return (self._shape_no_block, self._ranges_no_block, self.behavior, self._collapsed_su_no_block)
...
def detect_behavior(loop_body, loop_vars):
    """
    Detect loop behavior:
      - 'reduction': LHS appears in MAX/MIN call in RHS
      - 'accumulate': LHS appears in RHS AND the LHS subscripts are missing
                       a loop variable that the array actually uses (true reduction)
      - 'compute': default
    
    An in-place update like a(jc,jk) = a(jc,jk) - expr is NOT an accumulate
    because all loop vars present in the array's subscripts are covered.
    Also, a 2D array missing 'jb' is not a reduction — jb is simply not a
    dimension of that array.
    """
    has_accumulate = False
    for assign in FindNodes(Assignment).visit(loop_body):
        lhs_name = get_full_name(assign.lhs)

        # Collect full names of all variables on RHS
        rhs_names = set()
        for var in FindVariables().visit(assign.rhs):
            rhs_names.add(get_full_name(var))

        # LHS doesn't appear in RHS — pure compute
        if lhs_name not in rhs_names:
            continue

        # Check for MAX/MIN reduction
        rhs_str = str(assign.rhs).lower()
        if "max(" in rhs_str or "min(" in rhs_str:
            return "reduction"

        # LHS appears in RHS — check if it's a true reduction
        # (LHS subscripts missing a loop var that the array actually indexes by)
        if isinstance(assign.lhs, Array) and assign.lhs.dimensions:
            lhs_vars = set()
            for dim in assign.lhs.dimensions:
                for lv in loop_vars:
                    if expr_contains_var(dim, lv):
                        lhs_vars.add(lv)

            # For each loop var NOT in LHS subscripts, check if the array
            # actually uses that var in any of its accesses. If the array
            # simply doesn't have a dimension for that var (e.g. 2D array
            # without block dim), it's not a reduction.
            missing = set()
            for lv in loop_vars:
                if lv in lhs_vars:
                    continue
                # Check: does this array ever use this loop var in its subscripts?
                # We check ALL accesses of this array name in this assignment
                for var in FindVariables().visit(assign):
                    if not isinstance(var, Array):
                        continue
                    if get_full_name(var) != lhs_name:
                        continue
                    if var.dimensions:
                        for dim in var.dimensions:
                            if expr_contains_var(dim, lv):
...
def analyze_loop_nests(routine):
    ...
    for outer_loop in all_loops:
        ...
        for inner_loop in direct_children:
            ...
            behavior = detect_behavior(inner_loop.body, loop_vars)
    ...
    for loop in all_loops:
        ...
        behavior = detect_behavior(loop.body, loop_vars)
        if not accesses and behavior == "compute":
            continue
        nest = LoopNestInfo(
...
# ---------------------------------------------------------------------------
# Grouping (original — exact pattern match)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Layout grouping (collapsed — block-stripped, worst-case S/U per role)
# ---------------------------------------------------------------------------

def array_layout_key(array_name, nests):
    """
    Compute a layout-relevant key for an array by examining all its accesses
    across all nests.

    Returns (dim_roles, su_per_role):
      dim_roles  — tuple of roles the array is indexed by, excluding block,
                   in canonical order (h, v), plus 'extra' if there are
                   additional constant-indexed dimensions.
      su_per_role — tuple of worst-case S/U for each role in dim_roles.
                   'U' wins over 'S' (if any access is indirect, the role is U).

    Arrays with the same layout key have the same shape and the same
    structured/unstructured classification per dimension — they can share
    a memory layout without conflict.
    """
    role_worst = {}   # role -> worst S/U across all accesses
    roles_seen = set()
    max_dims_no_block = 0

    for nest in nests:
        for acc in nest.accesses:
            if acc.name != array_name:
                continue
            ndims_nb = 0
            for v, su in zip(acc.dim_vars, acc.structuredness):
                role = _role(v) if v and v not in ("const", "?") else None
                ...
                if su == "U":
                    role_worst[role] = "U"
                elif role not in role_worst:
                    role_worst[role] = "S"
            max_dims_no_block = max(max_dims_no_block, ndims_nb)

    # Canonical order
    order = {"h": 0, "v": 1}
    sorted_roles = tuple(sorted(roles_seen, key=lambda r: order.get(r, 99)))

    # Detect extra constant-indexed dimensions (e.g. ntnd slot)
    has_extra = max_dims_no_block > len(sorted_roles)
    if has_extra:
        dim_roles = sorted_roles + ("extra",)
        su = tuple(role_worst.get(r, "S") for r in sorted_roles) + ("C",)
    else:
        dim_roles = sorted_roles
        su = tuple(role_worst.get(r, "S") for r in sorted_roles)

    return (dim_roles, su)


def compute_layout_groups(nests):
    """
    Group all arrays by their layout key.

    Returns:
      layout_groups  — dict: layout_key -> sorted list of array names
      array_lkeys    — dict: array_name -> layout_key
    """
    all_arrays = sorted(set(a for nest in nests for a in nest.unique_array_names))
    array_lkeys = {}
    for a in all_arrays:
        array_lkeys[a] = array_layout_key(a, nests)

    layout_groups = defaultdict(list)
    for a in all_arrays:
        layout_groups[array_lkeys[a]].append(a)

    return dict(layout_groups), array_lkeys
...
def compute_set_analysis(nests):
    ...
    Returns a dict with:
      ...
      layout_groups     – dict: layout_key -> [array names]
      array_lkeys       – dict: array_name -> layout_key
    """
    n = len(nests)
    all_arrays = sorted(set(a for nest in nests for a in nest.unique_array_names))
    ...
    # For each array, collect the canonical set of roles it is indexed by.
    # E.g. (h, v) for a 2D array indexed by je/jc and jk.
    # We take the union across all accesses, ignoring "const"/"?" and block.
    array_dim_sig = {}
    for a in all_arrays:
        roles = set()
        for idx_nest, nest in enumerate(nests):
            for acc in nest.accesses:
                if acc.name != a:
                    continue
                for v, su in zip(acc.dim_vars, acc.structuredness):
        ...
        # Canonical order
        order = {"h": 0, "v": 1}
        sorted_roles = tuple(sorted(roles, key=lambda r: order.get(r, 99)))
        # Check for extra dimensions (constant-indexed dims like ntnd)
        max_dims_no_block = 0
        for idx_nest, nest in enumerate(nests):
            for acc in nest.accesses:
                if acc.name != a:
                    continue
                ndims = sum(1 for v in acc.dim_vars
                            if _role(v) != "b")
                max_dims_no_block = max(max_dims_no_block, ndims)
        has_extra = max_dims_no_block > len(sorted_roles)
        if has_extra:
            sorted_roles = sorted_roles + ("extra",)
        array_dim_sig[a] = sorted_roles

    # Group arrays by dimension signature
    dim_groups = defaultdict(list)
    for a in all_arrays:
        dim_groups[array_dim_sig[a]].append(a)

    # --- Max nest width ---
    max_nest_width = max(len(nest.unique_array_names) for nest in nests)

    # --- Layout groups (collapsed) ---
    layout_groups, array_lkeys = compute_layout_groups(nests)
    ...
    return {
        ...
        "nest_lhs": nest_lhs,
        "nest_rhs": nest_rhs,
        "array_dim_sig": array_dim_sig,
        "dim_groups": dim_groups,
        "max_nest_width": max_nest_width,
        "layout_groups": layout_groups,
        "array_lkeys": array_lkeys,
    }
...
def _layout_key_label(key):
    """Human-readable label for a layout key like (('h','v'), ('S','U'))."""
    dims, sus = key
    if not dims:
        return "(scalar) all-C"
    parts = []
    for d, s in zip(dims, sus):
        parts.append(f"{d}:{s}")
    dim_str = "(" + ", ".join(str(d) for d in dims) + ")"
    su_str = " ".join(parts)
    has_u = "U" in sus
    tag = "HAS UNSTRUCTURED" if has_u else "FULLY STRUCTURED"
    return f"{dim_str} {su_str} — {tag}"
...
def print_set_analysis(routine_name, nests, sa):
    ...
    # ── 3. Dimension Groups ──────────────────────────────────────────
    p("## 3. Dimension Groups\n")
    p("Arrays grouped by their dimension signature (block dimension stripped). "
      "Only arrays with identical shapes can share an AoS/AoSoA record.\n")

    dim_groups = sa["dim_groups"]
    for sig in sorted(dim_groups.keys(), key=lambda s: (-len(dim_groups[s]), s)):
        arrs = dim_groups[sig]
        sig_str = "(" + ", ".join(sig) + ")" if sig else "(scalar)"
        has_u = any(sa["array_class"][a] in ("bridge-U",) for a in arrs)
        tag = " ⚠ bridge-U" if has_u else ""

        p(f"### Signature `{sig_str}` — {len(arrs)} arrays{tag}\n")
        p("| Array | Class | S/U | Nests |")
        p("|-------|-------|-----|------:|")
        for a in sorted(arrs):
            cls = sa["array_class"][a]
            has_u_access = "U" if sa["array_u_nests"][a] else "S"
            n_nests = len(sa["array_to_nests"][a])
            p(f"| `{a}` | {cls} | {has_u_access} | {n_nests} |")
        p()

    # ── 4. Co-occurrence (top pairs) ─────────────────────────────────
    p("## 4. Array Co-occurrence (top pairs)\n")
    p("Pairs of arrays that appear together in the most nests.\n")
    ...
    p("## 5. Identical-Membership Clusters (Jaccard = 1.0)\n")
    p("Arrays that appear in exactly the same set of nests.\n")
    ...
    # ── 6. Nest overlap ──────────────────────────────────────────────
    p("## 6. Nest Overlap (Jaccard on array working sets)\n")
    p("Nest pairs ranked by array-set similarity.\n")
    ...
    p("These arrays are accessed with direct structured subscripts in some nests "
      "and via indirection in others.\n")
    ...
    # ── 9. Layout Groups (collapsed) ─────────────────────────────────
    p("## 9. Layout Groups (collapsed, block-stripped)\n")
    p("Arrays grouped by **(dimension signature, worst-case S/U per role)**. "
      "Block dimension stripped. Within a group, all arrays have the same shape "
      "and the same structured/unstructured classification per dimension.\n")
    p("This collapses the fine-grained \"exact access pattern\" groups into a "
      "small number of layout-equivalent classes.\n")

    layout_groups = sa["layout_groups"]
    array_lkeys = sa["array_lkeys"]

    # Sort groups: largest first, then by key
    sorted_lg = sorted(layout_groups.items(), key=lambda x: (-len(x[1]), x[0]))

    p(f"**{len(layout_groups)} layout groups** for {len(sa['all_arrays'])} arrays.\n")

    for key, arrs in sorted_lg:
        label = _layout_key_label(key)
        dims, sus = key
        has_u = "U" in sus
        tag = "⚠ UNSTRUCTURED" if has_u else "STRUCTURED"

        p(f"### {tag} — {label} ({len(arrs)} arrays)\n")
        p("| Array | Fine-grained class | Nests |")
        p("|-------|--------------------|------:|")
        for a in sorted(arrs):
            cls = sa["array_class"][a]
            n_nests = len(sa["array_to_nests"][a])
    ...
    # Summary table
    p("### Summary\n")
    p("| # | Dims | S/U | Tag | Arrays |")
    p("|--:|------|-----|-----|-------:|")
    for i, (key, arrs) in enumerate(sorted_lg, 1):
        dims, sus = key
        dim_str = ", ".join(dims) if dims else "scalar"
        su_str = " ".join(f"{d}:{s}" for d, s in zip(dims, sus))
        has_u = "U" in sus
        tag = "U" if has_u else "S"
        p(f"| {i} | ({dim_str}) | {su_str} | {tag} | {len(arrs)} |")
    p()

    # ── 10. Layout Conclusions ───────────────────────────────────────
    p("## 10. Layout Conclusions\n")

    max_w = sa["max_nest_width"]
    p(f"**SoA → AoS/AoSoA conversion is not indicated.** "
      f"The widest loop nest references {max_w} unique arrays. "
      f"Since no nest exceeds the typical register-pressure threshold of ~16 concurrent "
      f"streams, SoA (the current Fortran column-major layout) is not bandwidth-limited "
      f"by TLB or prefetcher capacity.\n")

    # Find Jaccard=1.0 pairs
    perfect_pairs = []
    for (a, b), jac in sa["jaccard_arrays"].items():
        if jac == 1.0:
            perfect_pairs.append((a, b))

    if perfect_pairs:
        p(f"**Co-access analysis:** only {len(perfect_pairs)} array pair(s) have "
          f"Jaccard = 1.0 (always accessed together):")
        for a, b in sorted(perfect_pairs):
            p(f"- `{a}` and `{b}`")
        p()
        p("These are the only candidates where AoS interleaving could improve spatial "
          "locality. All other pairs have Jaccard < 1.0, meaning they appear in different "
          "subsets of nests — interleaving them would waste bandwidth in nests that use "
          "only one member.\n")
    else:
        p("**Co-access analysis:** no array pairs have Jaccard = 1.0. "
          "AoS interleaving would waste bandwidth in every case.\n")

    p("**Blocking along the vertical dimension (nlev) is not beneficial.** "
      "The vertical dimension is small and constant (typically nlev = 90), so the "
      "full vertical column already fits in cache. Combined with indirect (unstructured) "
      "access patterns on the horizontal dimension, vertical blocking would add overhead "
      "without reducing cache misses.\n")
...
def print_single_block_view(routine_name, nests):
    ...
    for nest in nests:
        pattern_groups_nb[nest.pattern_key].append(nest)
