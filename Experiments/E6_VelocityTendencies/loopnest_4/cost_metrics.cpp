/*
 * cost_metrics.cpp -- loopnest_4 (Nest 24): indirect stencil + CFL-gated
 *                     diffusion, partial vertical jk in [jk_lo, jk_hi).
 *
 * 10 data arrays modelled:
 *   2D edge-shaped (5): DDT (RMW), VN, ZW, ZETA, DDQZ
 *   1D horizontal (5):  CLE (N_e*2), GF (N_e*5), AREA, TANG, INVP
 *
 * Indirect neighbour accesses through icidx/iqidx/ividx are NOT tracked:
 * their destinations are flat edge-ids whose reuse distances are
 * dominated by the ici/iqi/ivi patterns rather than layout choices, so
 * inclusion would add noise and mask the layout-sensitive signal from the
 * primary (je, jk) and horizontal (je) reference sets.
 */
#include <algorithm>
#include <climits>
#include <cmath>
...
enum ArrID { A_DDT=0, A_VN, A_ZW, A_ZETA, A_DDQZ,
             A_CLE, A_GF, A_AREA, A_TANG, A_INVP, NUM_ARR };
static const int eb[] = {8,8,8,8,8,8,8,8,8,8};
...
static inline bool is_horiz_1d(int a) {
  return a == A_CLE || a == A_GF || a == A_AREA || a == A_TANG || a == A_INVP;
}
...
inline void refs_v(int V, int je, int jk, int N, int nl, Ref r[NR]) {
  ...
  r[0]={A_DDT,  blk(c,A_DDT)};
  r[1]={A_VN,   blk(c,A_VN)};
  r[2]={A_ZW,   blk(c,A_ZW)};
  r[3]={A_ZETA, blk(c,A_ZETA)};
  r[4]={A_DDQZ, blk(c,A_DDQZ)};
  r[5]={A_CLE,  blk(je*2, A_CLE)};
  r[6]={A_GF,   blk(je*5, A_GF)};
  r[7]={A_AREA, blk(je,   A_AREA)};
  r[8]={A_TANG, blk(je,   A_TANG)};
  r[9]={A_INVP, blk(je,   A_INVP)};
}
...
Metrics compute_metrics(int V, int B, int W, int N, int nl, Schedule sc) {
  int jk_lo = nl/8;    if (jk_lo < 1) jk_lo = 1;
  int jk_hi = (7*nl)/8; if (jk_hi > nl-1) jk_hi = nl-1;
  LoopOrder lp = (V>0) ? iter_order(sc, V) : KLON_FIRST;
...
int main(int argc, char **argv) {
  ...
  std::string csv_def = "metrics_ln4_" + std::to_string(nl) + ".csv";
    ...
    for (int bi = 0; bi < NB; bi++)
      for (int si = 0; si < 2; si++)
        for (int ti = 0; ti < NT; ti++) {
          ...
          double ws = ((double)B * 10 * 8) / (double)L1_BYTES;
    ...
    for (int txi = 0; txi < 4; txi++)
      for (int tyi = 0; tyi < 4; tyi++)
        for (int ti = 0; ti < NT; ti++) {
          ...
          double ws = ((double)TX * 10 * 8) / (double)L1_BYTES;
  ...
  printf("\n  [ln4] N=%d nl=%d (partial vert [nl/8, 7*nl/8)) beta=%.4f alpha=%.4f gamma=%.3f P=%d\n\n",
