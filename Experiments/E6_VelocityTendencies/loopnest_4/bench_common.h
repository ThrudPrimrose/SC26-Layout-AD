...
static void fill_idx(int *__restrict__ arr, size_t n, int lo, int hi, unsigned seed) {
  for (size_t i = 0; i < n; i++) {
    uint64_t h = splitmix64((uint64_t)seed * 2654435761ULL + i);
    int range = hi - lo;
...
static void fill_mask(int *__restrict__ arr, size_t n, unsigned seed, double p_true = 0.6) {
  for (size_t i = 0; i < n; i++) {
    uint64_t h = splitmix64((uint64_t)seed * 2654435761ULL + i);
