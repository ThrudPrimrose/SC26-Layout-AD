/*
 * cost_metrics.cpp -- loopnest_2 (Nest 5): direct stencil, partial vertical.
 *   z_w_concorr_me[je,jk] = vn[je,jk]*ddxn[je,jk] + vt[je,jk]*ddxt[je,jk]
 * 5 arrays, zero indirection. Supports V1-V4, blocked, and (TX,TY) tiled.
 *
 * Partial vertical: jk runs over [nflatlev, nlev). For metrics we use the
 * same nflatlev = nlev/3 convention as the benchmark.
 */
#include <algorithm>
#include <climits>
#include <cmath>
...
static inline int nflat_for(int nl) { return nl / NFLATLEV_FRAC_DEN; }
...
/* ── arrays ── */
enum ArrID { A_OUT=0, A_VN, A_VT, A_DDXN, A_DDXT, NUM_ARR };
static const int eb[] = {8,8,8,8,8};
...
/* ── refs (5 arrays, same c) ── */
static constexpr int NR = 5;
struct Ref { int a; int64_t b; };
inline void refs_v(int V, int je, int jk, int N, int nl, Ref r[NR]) {
  int c = IC_v(V, je, jk, N, nl);
  r[0]={A_OUT,blk(c,A_OUT)}; r[1]={A_VN,blk(c,A_VN)}; r[2]={A_VT,blk(c,A_VT)};
  r[3]={A_DDXN,blk(c,A_DDXN)}; r[4]={A_DDXT,blk(c,A_DDXT)};
}
...
Metrics compute_slice(int V, int B, int W, int N, int nl, Schedule sc,
  ...
  if (B > 0) {
    ...
    for (int64_t ln = lo; ln < hi; ln++) {
      ...
      /* advance ln to next row boundary implicitly via loop; this trivial */
    }
  } else {
...
Metrics compute_metrics(int V, int B, int W, int N, int nl, Schedule sc) {
  int jk_lo = nflat_for(nl), jk_hi = nl;
  LoopOrder lp = (V>0) ? iter_order(sc, V) : KLON_FIRST;
...
int main(int argc, char **argv) {
  ...
  std::string csv_def = "metrics_ln2_" + std::to_string(nl) + ".csv";
    ...
    for (int bi = 0; bi < NB; bi++)
      for (int si = 0; si < 2; si++)
        for (int ti = 0; ti < NT; ti++) {
          ...
          double ws = ((double)B * 5 * 8) / (double)L1_BYTES;
    ...
    for (int txi = 0; txi < 4; txi++)
      for (int tyi = 0; tyi < 4; tyi++)
        for (int ti = 0; ti < NT; ti++) {
          ...
          double ws = ((double)TX * 5 * 8) / (double)L1_BYTES;
