...
int main(int argc, char *argv[]) {
  ...
  HIP_CHECK(hipMalloc(&d_vnie, sz_max*8));
  HIP_CHECK(hipMalloc(&d_zvt,  sz_max*8));
  HIP_CHECK(hipMalloc(&d_zk,   sz_max*8));
  HIP_CHECK(hipMalloc(&d_vn,   sz_max*8));
  HIP_CHECK(hipMalloc(&d_vt,   sz_max*8));
