#!/usr/bin/env bash
# Submit run_beverin.sh for every E6 subtask (loopnest_1..6 + full_velocity_tendencies).
...
#
# Usage:  bash submit_e6_beverin.sh [L1 L2 ...]
...
#       bash submit_e6_beverin.sh L1 L2 L3 L4 L5 L6 FVT
#       bash submit_e6_beverin.sh FVT          # just the full module
#
# Env overrides:
#   DRY_RUN=1  print the commands instead of running them.
#
# Notes:
#   - L1..L6 are pure C++/HIP microbenchmarks; they don't import dace and
...
jobs_log="${SCRIPT_DIR}/.submit_e6_beverin_jobs.log"
: >"${jobs_log}"
...
for tag in "${targets[@]}"; do
    ...
    script="${E6_ROOT}/${dir}/run_beverin.sh"
    if [[ ! -f "${script}" ]]; then
    ...
    if [[ "${DRY_RUN:-0}" == "1" ]]; then
        echo "[dry-run] (cd ${E6_ROOT}/${dir} && sbatch run_beverin.sh)"
        continue
    fi
...
if [[ "${DRY_RUN:-0}" != "1" ]]; then
    echo
    echo "[submit_e6_beverin] queue snapshot:"
