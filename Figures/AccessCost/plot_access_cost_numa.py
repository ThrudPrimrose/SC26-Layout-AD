...
def draw_numa_cost(values, ribbon_order, ribbon_label, numa_boundary_block, outname):
    fig, ax = plt.subplots(1, 1, figsize=(1.8, 1.8))
    ax.text(0.115-0.035, 1.01, r'$\Delta_\omega$', transform=ax.transAxes,
            ha='center', va='bottom', fontsize=10, color='#444')
    ax.text(0.205-0.035, 1.04, '(Weighted Block Distance)', transform=ax.transAxes,
    ...
    if "tiled" in outname:
        for x_val in [0, 2, 4]:
            ax.plot([grid_x0 + x_val * cell, grid_x0 + x_val * cell], [0, nrows * cell],
                    ls='--', color='#aaaaaa', lw=0.5, dashes=(3, 3))
        for y_val in [0, 2, 4]:
            ax.plot([grid_x0, grid_x0 + ncols * cell], [y_val * cell, y_val * cell],
                    ls='--', color='#aaaaaa', lw=0.5, dashes=(3, 3))
    else:
        for x_val in [0, 4]:
            ax.plot([grid_x0 + x_val * cell, grid_x0 + x_val * cell], [0, nrows * cell],
                    ls='--', color='#aaaaaa', lw=0.5, dashes=(3, 3))
        for y_val in [0, 1, 2, 3, 4]:
            ax.plot([grid_x0, grid_x0 + ncols * cell], [y_val * cell, y_val * cell],
                    ls='--', color='#aaaaaa', lw=0.5, dashes=(3, 3))

    # --- NUMA boundary line (between row 1 and row 2 in grid) ---
    numa_y = 2 * cell
    ax.plot([grid_x0, grid_x0 + ncols * cell], [numa_y, numa_y],
            ls='-', color='#cc3333', lw=1.8, alpha=0.8)

    # NUMA labels
    ax.text(grid_x0 - 0.16, 3 * cell, 'NUMA 0', ha='center', va='center',
            fontsize=6.8, color='#cc3333', rotation=90)
    ax.text(grid_x0 - 0.16, 1 * cell, 'NUMA 1', ha='center', va='center',
            fontsize=6.8, color='#cc3333', rotation=90)

    # --- Inner cells with rho^omega values ---
    for r in range(nrows):
        for c in range(ncols):
    ...
    for addr, (r, c) in enumerate(ribbon_order):
        color = cell_color(r, c)
    ...
    # NUMA boundary in ribbon
    numa_x = ribbon_x0 + numa_boundary_block * 2 * elem_w
    ax.plot([numa_x, numa_x], [ribbon_y - 0.05, ribbon_y + ribbon_h + 0.05],
            ls='-', color='#cc3333', lw=1.8, alpha=0.8)
...
ribbon_rowmajor = [(r, c) for r in range(nrows) for c in range(ncols)]
draw_numa_cost(values_rowmajor, ribbon_rowmajor,
               'Row-major Layout', numa_boundary_block=4,
               outname='numa_cost_rowmajor')
...
ribbon_tiled = []
for tr in range(nrows // 2):
    for tc in range(ncols // 2):
        for lr in range(2):
            for lc in range(2):
                ribbon_tiled.append((tr * 2 + lr, tc * 2 + lc))
draw_numa_cost(values_tiled, ribbon_tiled,
               '2×2 Tiled Layout', numa_boundary_block=4,
               outname='numa_cost_tiled')